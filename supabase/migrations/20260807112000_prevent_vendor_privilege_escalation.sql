...
--   により管理者へ昇格できてしまう。
--
--   さらに "vendor insert self" ポリシー（20260224011000）も
--     with check (auth.uid() = id)
--   のみで role 列に制限がないため、まだ vendors 行を持たない任意の認証済み
--   ユーザーが
--     POST /rest/v1/vendors  { "id": "<自分のauth.uid()>", "role": "admin", ... }
--   を直接叩くだけで管理者として行を新規作成できてしまう（INSERT経路）。
--
-- 対策:
--   BEFORE INSERT OR UPDATE トリガで role / must_change_password を保護する。
--   - UPDATE: 変更を拒否する（従来どおり）。
--   - INSERT: 値を検証せず、既定値（'vendor' / true）へ強制的に上書きする。
--     出店者が自分で行を作成する経路（サインアップ）は "vendor" ロールの
--     行が作られることだけを期待しており、role を自由記述できる必要はない。
...
--   service_role と DB 管理者だけはこれまでどおり自由に変更できる。
--
--   関数は NEW/OLD の列比較と raise exception のみを行い、定義者以外の権限を
--   必要としないため security definer にはしない（最小権限の原則、デフォルトの
--   security invoker のままにする）。
...
-- 目的: 出店者が自分の vendors 行の INSERT/UPDATE を通じて
--       管理者権限を取得できる経路を塞ぐ。
