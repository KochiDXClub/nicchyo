...
/**
 * 管理画面の対話テスト（app/api/admin/ai-models/test）から呼ぶときの指定。
 *
 * 公開APIの経路（route.ts）からは渡さない。渡すと以下が変わる。
 * - 使うモデルを台帳の設定ではなく modelOverride にする
 * - origin 検査・レート制限・不正検知を飛ばす（管理者として認可済みの内部呼び出しのため）
 * - ai_consult_logs に相談ログを残さない（来訪者の相談ではないので集計を汚さない）
 * - OpenAI の失敗内容を debugError として返す（運営が原因を見られるように）
 */
export type ConsultAskInternalOptions = {
...
async function finalizeConsultResponse(options: {
  ...
  internal?: ConsultAskInternalOptions;
  ...
  if (internal) {
    // 管理画面の対話テスト。来訪者の相談ではないので集計に混ぜない
  } else if (logVendorIds.length > 0) {
...
async function createStreamingConsultResponse(options: {
  ...
  const aiModel = internal?.modelOverride ?? (await resolveAiModelFor("consult"));
    ...
    return NextResponse.json(
      {
        ...
        ...(internal ? { debugError: (await readOpenAiError(upstream)).summary } : {}),
      },
      { status: 500 },
    );
  }
...
export async function handleConsultAsk(
  request: Request,
  internal?: ConsultAskInternalOptions,
) {
  try {
    ...
    // 不正検知は来訪者向け。管理者の対話テストで管理者のIPを疑わせない
    if (supabaseUrl && serviceRoleKey && !internal) {
    ...
    if (!embeddingResponse.ok) {
      const detail = await readOpenAiError(embeddingResponse);
      console.error(`[grandma/ask] embeddings failed: ${detail.summary}`);
      return NextResponse.json(
        {
          ...buildErrorResponse(
            "system_error",
          ...
          ...(internal ? { debugError: detail.summary } : {}),
        },
        { status: 500 },
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          ...(internal ? { debugError: (await readOpenAiError(chatResponse)).summary } : {}),
        },
        { status: 500 },
      );
    }
