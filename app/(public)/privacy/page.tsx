...
const destinations: Destination[] = [
  ...
  },
  {
    ...
    service: "AI相談・マップの案内",
    sends:
      "ご相談で入力・お話しいただいた内容、ご案内に必要な店舗の情報、および位置情報を許可されている場合はそのときの緯度・経度",
    purpose: "ご相談への回答文の作成",
  },
  {
      ...
      <main className="mx-auto max-w-[38rem] px-5">
        ...
        <div className="border-l-[3px] border-nicchyo-primary pl-5 sm:pl-6">
          <p className="text-[1.05rem] font-bold leading-[1.9] text-nicchyo-ink sm:text-[1.15rem]">
            地図に現在地を表示するだけであれば、位置情報は端末の外に出ません。
            AI に相談されたときだけ、近くのお店をお探しするためにお預かりします。
          </p>
        </div>
        ...
        <div className="mt-14 space-y-14">
          ...
          <Section number={2} title="位置情報の取り扱い">
            ...
            <Prose>
              一方、<strong className="font-semibold text-nicchyo-ink">AI相談やマップの AI
              にご相談いただいたときは</strong>、そのときの緯度・経度を当サービスのサーバーへ送信し、
              回答を作成するために OpenAI にも渡しております。近くのお店から順にお探しし、
              「会場にいらっしゃるのか、これから向かわれるのか」に合わせてご案内を変えるために用いるものです。
            </Prose>
            <Prose>
              送信した緯度・経度そのものを保存することはございません。相談の記録として残るのは、
              「会場の近くにいらっしゃったかどうか」という区分だけです。
            </Prose>
            <Prose>
          ...
          <Section number={3} title="外部への送信">
            ...
            <p className="mt-6 text-sm leading-[1.9] text-[#8C8378]">
              緯度・経度が含まれるのは、AI にご相談いただいたときの OpenAI への送信だけです。
              地図の背景を配信する事業者には、表示している地図の範囲が伝わります。
              AI への送信は当サービスのサーバーを経由して行っており、ご相談の内容は回答を作成するためにのみ用います。
            </p>
          </Section>
