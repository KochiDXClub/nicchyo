...
<table style="width:100%;border-collapse:collapse;margin:16px 0;">
  <tr><td style="padding:8px;background:#f8f8f8;font-weight:bold;width:100px;">対象</td><td style="padding:8px;">${params.targetName}</td></tr>
  <tr><td style="padding:8px;background:#f8f8f8;font-weight:bold;">理由</td><td style="padding:8px;">${params.reason}</td></tr>
  ${params.details ? `<tr><td style="padding:8px;background:#f8f8f8;font-weight:bold;">詳細</td><td style="padding:8px;">${params.details}</td></tr>` : ""}
</table>
...
export function inquiryReceivedTemplate(params: {
  ...
  const safeMessage = params.message.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");

  const html = `
<!DOCTYPE html><html lang="ja"><body style="font-family:sans-serif;max-width:600px;margin:0 auto;padding:20px;">
<h2 style="color:#e5620a;">✉️ 新しいお問い合わせが届きました</h2>
<table style="width:100%;border-collapse:collapse;margin:16px 0;">
  <tr><td style="padding:8px;background:#f8f8f8;font-weight:bold;width:100px;">送信者</td><td style="padding:8px;">${params.name ?? "匿名"} &lt;${params.email}&gt;</td></tr>
  <tr><td style="padding:8px;background:#f8f8f8;font-weight:bold;">カテゴリ</td><td style="padding:8px;">${params.category}</td></tr>
