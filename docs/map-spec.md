...
*   **`app/(public)/map/`（MapPageClient.tsx がハブ）**
    ...
    *   AI相談の起点は画面下部 `NavigationBar` の「相談」ボタン（`onConsultClick`）のみ。押すと `/consult` に遷移する。マップ内蔵の `MapCharacterConsult`（地図上に相談UIを重ねる実装）は既にコードとして存在するが、有効化する導線（`mapCharacterConsultActive` を true にする箇所）が現状無く、実質デッドパス。（`MapAgentAssistant` と `/api/map-agent` は到達不能なまま二重実装になっていたため、[#509](https://github.com/KochiDXClub/nicchyo/issues/509) の判断にもとづき削除しました。）`MapCharacterConsult` を削除するか有効化するかは、引き続き [#509](https://github.com/KochiDXClub/nicchyo/issues/509) で判断します。
